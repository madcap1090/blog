...
sleep_sec <- 16 * 60    # 15 minutes is the time window for rate limit reset; add a
#little time buffer of 1 min.
chunksize <- 100        # number of user IDs per request; the docs say 300 requests with
#100 IDs each per 15 min.
n_max_requests <- 280   # max. number of requests within a 15 min. time frame; we stay a
#bit below the 300 requests threshold
chunk_idx <- 0          # current chunk index
cur_retry <- 0          # current number of retries; is reset to 0 once a successful API
#call was made
...
The 'INDEP' is Jean-Marie Dedecker who ran on a N-VA list. So the high pr option of links to the N-VA is not surprising. What is interesting on the other hand is that people from the cdH tend to follow people from the MR more than they do people from their own party.  
...
So the president of the Flemish fascist party seems to be an interesting node to inspect, because having relatively lower degree scores (total being 46) his account has a very high betweenness score. Find out more [here](https://igraph.org/r/doc/betweenness.html).  
...
```
 ...
 Setting the vertices and edge colors according to party membership
...
[![small_pic]("./img/20190728/graph_small.png")]("http://google.be")


Obviously too small. But you can look at a larger image [here](). The colors of the incoming arows indicate which party they are from and 

https://stackoverflow.com/questions/42235301/how-to-hyperlink-an-image-in-r-markdown

and if memory serves me right put the image and html file (?) in static?

And now for the interactive (html) plot, with the visNetwork package.
...
# https://badtothecode.netlify.com/post/files/20190728/friends/
# there is somting there
```
