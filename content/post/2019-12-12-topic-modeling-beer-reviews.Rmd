...
***
The last three words in the review are probably inserted automatically; the user name, the number of characters, and month of the review. So we can calculate the number of reviews per user and then filter out these last three words too. 
...
So these top ones have been quite prolific. Looking at the number of reviews published by all the users (were still only on the Belgian beers only by the way) we get a long-tail shape: 
...
df_users <- users %>% 
  ...
  summarise(count = n()) %>% 
  arrange(desc(count)) %>% 
  filter(count > 1 & count < 400)

ggplot(df_users, aes(x=reorder(user, -count), y=count)) + 
...
We can clean the text further, only keeping English reviews,  and removing the usernames and the months from the reviews, and get rid of small or empty reviews :
...
vocabulary %>% arrange(desc(term_count)) %>%
  slice(1:1250) %>% 
  #datatable(rownames = FALSE,  options = list(dom  = 't'),class = "compact")
...
We know we have between 15 and 20 big beer type categories from the previous post. So we might reasonably assume the optimal number of topics might be in the same range. There are however more objective ways to calculate the optimal number of topics. Futher information can be found in [this](http://text2vec.org/topic_modeling.html) article of Dmitry Selivanov. 
...
```{r}
saveRDS(lda_model, "./data/20191212/lda_model.rds")
lda_model <- readRDS("./data/20191212/lda_model.rds")
```
...
temp <- lda_model$get_top_words(n = 15, topic_number = c(1:15), lambda = 0.4) %>% 
  as.data.frame()
...
http://127.0.0.1:4321/post/20191212/ldavis/#topic=3&lambda=0.4&term=
...
So this review scores the highest in topics 6, 9 and 11. The review points to a darker, stronger brew. 
...
Topic eight for instance seems to be the topic of the reviews of beers people liked:

```{r}
df_beer_type %>% 
  select('8', beer_name, brewery, review) %>% 
  arrange(desc(`8`)) %>% 
  select(-'8') %>% 
  slice(1:25) %>% 
  datatable(class = "compact")
...
So would there be a topic regrouping negative reviews? Seems to be topic 12, but it is not that pronounced as the positiveness in topic 8. 

```{r}
df_beer_type %>% 
  select('12', beer_name, brewery, beer_type, review) %>% 
  arrange(desc(`12`)) %>% 
  select(-'12') %>% 
  slice(1:25) %>% 
  datatable(class = "compact")
...
So for topic one, the sum of the scores of the highest 10% are the highest for Belgian Tripel style of beer. 
...
saveRDS(df_top, "./data/20191212/temp_mean.rds")

df_top <- readRDS("./data/20191212/temp_mean.rds")
...
We obtain per topic and per beer type the sum of the topic scores of the reviews with the highest scores per topic. And that's a bit of a mouthful, but we can plot them (also excluding the smaller topics).
...
ggplot(data = df, aes(x = topic, y= beer_type)) +
  geom_point(aes(color= factor(beer_type)),size = df$topic_score/150, alpha = 1/4) +
  theme_minimal() +
  theme(legend.position = "none") +
   coord_cartesian(xlim =c(0.8, 16), ylim =c(0, 16)) +
...
I like this way of presenting things even if interpretation needs to be a bit careful. The plot makes the topics a a bit more palatable as it were. 

There is still is a s*** avenues to explore with topic modeling and I'm sure we will get back to them, but this post is already a bit too long so concluding with the mapping of the 20 most commented beers with the same analysis frame.
...
ggplot(data = df, aes(x = topic, y= beer_name)) +
  geom_point(aes(color= factor(beer_name)),size = df$topic_score/25, alpha = 1/4) +
  theme_minimal() +
  theme(legend.position = "none") +
  coord_cartesian(xlim =c(1, 15.5), ylim =c(0, 20)) +
  scale_x_continuous(breaks = seq(1, 15, by = 1)) +
  geom_vline(xintercept = 1:15, alpha = 0.15) +
  labs(y=NULL) 
```

So here again we can see which beers are in the highly rated topic (8). 
