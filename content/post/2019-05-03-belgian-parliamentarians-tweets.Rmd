...
But regexes are always more difficult than they seem. So here it is just easier to use 'substr'.
...
So let's bring them all together before tackeling missing values.
...
For now I leave some field incomplete and see if we can have fun finding them with algorythms or with some other tools. But since we have a number of their tweets in ram, we kan have a shot at guessing their language 'role'.
