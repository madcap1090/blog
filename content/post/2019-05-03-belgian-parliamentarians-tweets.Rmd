...
z <- y %>% 
  mutate(digits = str_extract_all(Opmerkingen,"([0-9]+)")) #+ keeps them togethe
test <- y %>% 
  mutate(digits = str_remove_all(Opmerkingen,"([^0-9]+)")) #+ keeps them togethe

gsub(",","",str_extract_all(y$Opmerkingen,"(^[0-9]"))


w <- z %>% 
  mutate(num = paste0(digits, collapse = ""))
