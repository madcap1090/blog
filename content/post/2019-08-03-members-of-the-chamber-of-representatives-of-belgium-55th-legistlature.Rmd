...
We are one good month into Belgiums 55th parlamentairian legistlature and the chamber's website has been updated, so it is time to get to know our representatives just a litte bit better. 
...
A first table is scraped with their name, party and sometimes email.
...
'table_ids' can now be disregarded. And we can join the information we have on a key constructed with the lastnames and firstnames, keeping in mind that the urls and ids are correct in the 'names' data frame.
...
saveRDS(names, "./data/20190802/reps_55.rds")
```
...
map2(.x = reps_55$ln_fn, .y = reps_55$id, safely(.f = download_gifs_with_0, otherwise = NULL))
...
for(i in gifs_needed){
  print(paste0("./data/20190802/down_2/",i, ".gif"))
  pic <- image_read(paste0("./data/20190802/down_2/",i, ".gif"))
  image_write(pic, path = paste0("./data/20190802/down_1/",i,".jpg"), format = "jpg")
}
...
```{r, results=FALSE}
dir.create("./data/20190802/down_3")
two_missing <- reps_55 %>% 
...
map2(.x = two_missing$ln_fn, .y = two_missing$id, safely(.f = download_gifs_no_0, otherwise = NULL))
# treat the è
...
image_write(pic, path = paste0("./data/20190802/down_1/Van Hoof Els.jpg"), format = "jpg")
pic <- image_read("./data/20190802/down_3/Wilmes Sophie.gif")
image_write(pic, path = paste0("./data/20190802/down_1/Wilmès Sophie.jpg"), format = "jpg")
...
list.files("./data/20190802/num_rep")

file.rename(list.files("./data/20190802/num_rep"), 
            paste0(1:150,".jpg"))
...
files <- sample(files, 150) # 176 will make a nice patch
...
It looks like they finaly voted to legalise it, but are laughing because they did not tell us.
