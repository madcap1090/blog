...
venues <-  readRDS(venues, "./data/20190524/venues.rds")

```


Either they got real creative or the variable 'type' is not very usefull:
...
Converting lat and lon (gives a warning because of the venue in Antartica if it is not sliced away).
...
venues_coord <- venues %>% 
  ...
  mutate(NS = str_extract(lat, "[[:alpha:]]"),
         ...
         lat = ifelse(str_detect(venue, "Antartica"), "-62.2381424", lat),
         lon = ifelse(str_detect(venue, "Antartica"), "-58.6688106", lon),
         lat = str_remove(lat,'°'),
         lon = str_remove(lon,'°'),
...
The darker the green the more recent the concert. They mainly play North America and Europe. But as we saw also in Antartica and in Tuktoyaktuk in Canada.
