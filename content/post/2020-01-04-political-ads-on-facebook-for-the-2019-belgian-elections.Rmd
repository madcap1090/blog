...
Russia critising a possible withdrawal of us is troops from Afghanistan is such a to 
...
So the Vlaams Belang takes first and second place, bearing in mind that the European Parliament spreads its ads over a much wider range of regions (hunderds, and there are three in Belgium). 

And here is an estimation of what was spent in roubles on these De Wever ads:
...
What was the reach of these ads/pages? Or rather we have a range of impressions per ad (the number of times the content was diplayed to a target audience. 
...
This means that the ads of the N-VA and sp.a have had a bigger number of impressions than the page of Tom Van Grieken, despite having spent less money on them. Having said that reach and clicks are also important metrics to evaluate marketing impact, but we just do not have these data. 
...
Unless of course you see selling their water solution for whiping polution from your face as an environmental cause...

One strategy to get only the polital actors is to filter on the pages or funding enteties that have a political party name in them. But some politicians do not mention in their page name for which party they are a candidate for. So all the candidates names are also needed. 

Let's do some scraping from an official election site.

```{r eval=FALSE, include=TRUE}
url <- "https://elections2019.belgium.be/en/candidates?title=&field_election_id_value=&field_party_name_value=&order=field_party_name&sort=asc&page=0"

candidates <- data.frame()
selector <- "body > div > main > div > div > section > div > div.views-element-container.form-group > div > div.view-content > div > table"

for(i in (0:138)){
  print(i)
  url <- glue("https://elections2019.belgium.be/en/candidates?field_election_id_value=&field_party_name_value=&title=&page={i}")
  table <- url %>% 
  read_html() %>% 
  html_node(selector) %>% 
  html_table()
...
```{r}
more <- c("Jean-Claude Marcourt - Officiel", "Patrick Cottenie","Guillaume Soupart - Conseiller communal", 
          "Cédric Mahieu - Conseiller communal à 1030", "Hannes De Reu", "Alexander Vandersmissen" ,
          "Marie Arena","Luc Haegemans", "Michael Freilich - Kamerlid", "Christophe Leurident", 
          "Lara Thommès - Conseillère communale à Ganshoren", "Kevin Engelen","Marie Meunier", 
          "Thomas Van Hoof")
...
There is a data frame based on the candidates, and another one based on the parties. Time to put them together. And we will be focussing on 2019.
...
i tried to find the most interesting insights and present it in this blog pots. from th beginning i felt that the data was interesting and really fun to play with. but what is more salient i think, is that there really is a transparency deficit. 


The Vlaams Belang seems to be spending more than three times more than the second biggest spender, the N-VA. 
...
reactable(top_df, columns = list(
  page_name = colDef(width = 80),
  name_party = colDef(width = 80),
...
Only in week 40 was there some advertising from the PVDA and PTB.
...
There are also some demographical data at hand. But they need to be extracted from a column. Here under with a bit of a slow loop but I have to go and run errands anyway.
...
```{r}
library("details") # install.packages("details")
details::details({
  x <- 1
  x * 2
...
```
Jason Kint (@jason_kint) tweeted at 5:28 am on Sun, Jan 05, 2020:
critical to understand based on these documents that it's not simply the existence of demonstrably false ads which makes Facebook toxic to free and fair elections. It's the allowance of these ads to be hypertargeted (eg microtargeting disinfo towards neurotic voters).

https://www.theguardian.com/technology/2020/jan/09/facebook-political-ads-micro-targeting-us-election?CMP=Share_AndroidApp_Gmail
