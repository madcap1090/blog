...
So that seems to work. Unfortunately while scraping it is not infrequent to encounter coding inconsistencies. For instance member "00902" does not have a "00902.gif" his is "902.gif". Dirty data :alarm:, shame on you webmaster :wink:. 
...
download_gifs_no_0 <- function (name, id){
  id <- as.numeric(id)
  url_pic <- glue("http://www.dekamer.be/site/wwwroot/images/cv/{id}.gif")
  download.file(url_pic, destfile = glue("./data/190417/temp/{name}.gif"), mode = "wb")
}
...
# safely(map2(.x = table_ids$name, .y = table_ids$id, .f = download_gifs_with_0), otherwise = NULL)
...
length(list.files("./data/190417"))-1
...
length(list.files("./data/190417/temp"))

```

Hmm so now we have too many pictures (and the ones with na leading 0 seem older). And it seems some are still missing :unamused:.


![Old Mr Calvo](/img/20190417/Calvo Kristof_06128_old.gif)

![Young Mr Calvo](/img/20190417/Calvo Kristof_young.gif)

Which new pics were obtained?

```{r}
new <- list.files("./data/190417")
old <- list.files("./data/190417/temp")
...
```{r message=FALSE, warning=FALSE, include=FALSE}
# file.copy(from= new_pics, to="./content/post/data/190417", 
#           overwrite = recursive, recursive = FALSE, 
#           copy.mode = TRUE)

lapply(new_pics, function(x) file.copy(paste ("./data/190417/temp", x , sep = "/"),  
...
```{r}
unlink("./data/190417/temp", recursive = TRUE) # don't need those anymore
