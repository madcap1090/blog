...
No proof of 'information operation' here.
...
IMHO there is no proof here at all. Maybe lat & lon ?

```{r}
unique(china_tweets$latitude)
```

So some latitudes are present. 

```{r}

library("tidyverse")

locations <- china_tweets %>% 
  filter(latitude == "present")
```

Ah well, so the geolocation is 'present' but not shared. We'll have to trust in Twitter that they originated from China. 
