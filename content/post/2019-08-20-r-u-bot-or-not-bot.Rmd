...
I find it an interesting turn of phrase because saying that an action undermines the legitimacy of an action just falls short of calling the action legitimate. But what I find most interesting is to know why Twitter decided to tackle 'information operations' targeting protest actions in Hong Kong. 

The couple of screenshots and the four files that Twitter let's us see, do not illustrate in any way that these accounts were involved in 'coordinated inauthentic behaviour', from a 'state-backed' operation.  

They do indicate that some of the accounts have a tendency to abuse substances. But that is about it. 
...
Who decides when accounts are behaving in a legitimate matter or not? Using which hard criteria? Hard ones and not some wooly [ones](https://help.twitter.com/en/rules-and-policies/platform-manipulation) By these criteria would the account of the orange menace not need to be suspended? Does that account not 'attempting to sow political discord'? Who decides? How?
...
In this, the largest of its kind in the industry, 'information operation', much more transparency from Twitter is requered. 
