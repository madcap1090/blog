...
In an earlier post we scraped the pictures of parliamentarians and used Recognition to analyses their faces. It's time to look a little bit clorser at the data, test the usage of the 'gather' and 'spread' function and get to know ggplot a little bit more.
...
Here are the different features we now have available. On the side of the observations (rows), it might be best to take a snapshot (at the end of the legistlation) in order to have a slightly more objective representation. 
...
What is our gender inbalence here? 
...
<center>
![Mrs Frenandez](/img/20190514/63.jpg)  ![Mrs Lijnen](/img/20190514/104.jpg)  ![Mrs Goffinet](/img/20190514/80.jpg)

</center>


Looking at the confidence values ReKognition is pretty sure about the results and seems to be very accurante. But how accurate? A mosaic of the pictures identified as women will create a quick visual and will enable us to have a fast visual check. 
...
Regognize seems to have calculated corectly:
...
So the emotions are a bit more complicated here. They usualy are. And if they aren't mixed, they probably are not emotions. This time we have some more emotions.confidence though. 
...
```{r}
p4 <- ggplot() + geom_bar(aes(y = count, x = party, fill = happy), data = charts.data,
                           stat="identity")
p4
```
...
```{r}
gather(charts.data, "emotion", "score", 2:8)
```

```{r}
#charts.data <- gather(charts.data, "emotion", "score", 2:8)
p4 <- ggplot() + geom_bar(aes(y = score, x = party, fill = emotion), data = charts.data,
                           stat="identity")
p4
...
p4 <- ggplot() + theme_economist() + scale_fill_economist() +
  ...
  geom_bar(aes(y = score, x = party, fill = emotion), data = charts.data,
                           stat="identity") +
  #geom_text(data=charts.data, aes(x = party, y = score, label = paste0(score,"%")),
